    ...
    (format, http, query, record, runtime, search, serverWidget) => {
        ...
        const onRequest = (scriptContext) => {
            try{
                ...
                form.clientScriptModulePath = './nsacs_cs_bookingsandshipmentsreport_v3.js';
        ...
        function htmlBookingsReportBody(reportEndingDate, totalMonthWorkDays, mtdWorkDays, fullMonthBudget){
            ...
            var totalBookings = 0;
            var totalMtdAvg = 0;
            var totalMtdBookings = 0;
            var totalRunRateBookings = 0;
            var totalVarPercentageVsAct = 0;
            var totalVarAmountVsAct = 0;
            var totalTotalBacklog = 0;
            var totalCurrentMonthReleasedBacklog = 0;
            var totalPendingApprovalBacklog = 0;
            var totalPastDue = 0;
            ...
            salesorderSearchObj.run().each(function(result){  
                ...
                if(mtdBookings != 0){
                    var salesorderSearchObj = search.create({
                        ...
                        [
                            ...
                            "AND", 
                            ["formulanumeric: {quantity}-NVL({quantityshiprecv},0)","greaterthan","0"], 
                            "AND", 
                            ["shipping","is","F"], 
                            "AND", 
                            ["taxline","is","F"], 
                        ...
                        columns:
                        [
                            search.createColumn({name: "formulacurrency", summary: "SUM", formula: "CASE WHEN {status} = 'Pending Fulfillment' THEN ROUND({amount}/1000) ELSE 0 END", label: "Curr M Released Backlog"}),
                            search.createColumn({name: "formulacurrency", summary: "SUM", formula: "CASE WHEN {status} = 'Pending Approval' THEN ROUND({amount}/1000) ELSE 0 END", label: "Pending Approval Backlog"})
                        ]
                    });
                    ...
                    salesorderSearchObj.run().each(function(result){
                        ...
                        pendingApprovalBacklog = resultJson.values['SUM(formulacurrency)_1'];
                        totalBacklog = Number(currentMonthReleasedBacklog) + Number(pendingApprovalBacklog);
                        //return true;
                    });                                  
                }
                ...
                var salesorderSearchObj = search.create({
                    ...
                    [
                        ...
                        ["class","anyof","@ALL@"], 
                        "AND", 
                        ["shipdate","before",,reportEndingDate], 
                        "AND", 
                        ["location","anyof",locationRecordId]
                    ],
                    columns:
                    [
                        search.createColumn({name: "formulacurrency", summary: "SUM", formula: "CASE WHEN {status} = 'Pending Fulfillment' THEN ROUND({amount}/1000) ELSE 0 END", label: "Past Due"})
                    ]
                });
                ...
                if(mtdAvg != 0){   
                    ...
                    runRateBookings = mtdAvg * totalMonthWorkDays;
                }                
                ...
                html += "<td class='bottomborder' align='right'></td>";
                html += "<td class='bottomborder' align='right'></td>";
                html += "<td class='bottomborder' align='right'></td>";
                html += "<td class='bottomborder' align='right'></td>";
                ...
                totalBookings = totalBookings + Number(bookings);
                totalMtdAvg = totalMtdAvg + Number(mtdAvg);
                totalMtdBookings = totalMtdBookings + Number(mtdBookings);
                totalRunRateBookings = totalRunRateBookings + Number(runRateBookings);
                totalTotalBacklog = totalTotalBacklog + Number(totalBacklog);
                totalCurrentMonthReleasedBacklog = totalCurrentMonthReleasedBacklog + Number(currentMonthReleasedBacklog);
                totalPendingApprovalBacklog = totalPendingApprovalBacklog + Number(pendingApprovalBacklog);
                totalPastDue = totalPastDue + Number(pastDue);

                return true;
            });

            var totalVarPercentageVsActLine = '';

            //if(mtdBudget && totalMtdBookings != 0){
            if(mtdBudget && totalMtdBookings >= 1){                    
                totalVarPercentageVsAct = ((mtdBudget - totalMtdBookings)/mtdBudget)*100;
                totalVarAmountVsAct = (mtdBudget - totalMtdBookings);

                totalVarPercentageVsActLine = formatTotalNumber(totalVarPercentageVsAct) + ' %';
            }

            html += "<tr>";
            html += "<td class='bottomborder'><strong>TOTAL</strong></td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalBookings) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalMtdAvg) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalMtdBookings) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalRunRateBookings) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(fullMonthBudgetNew) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(mtdBudget) + "</td>";
            html += "<td class='bottomborder' align='right'>" + totalVarPercentageVsActLine + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalVarAmountVsAct) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalTotalBacklog) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalCurrentMonthReleasedBacklog) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalPendingApprovalBacklog) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalPastDue) + "</td>";
            html += "</tr>";
        ...
        function htmlShipmentsReportBody(reportEndingDate, totalMonthWorkDays, mtdWorkDays, fullMonthBudget){
            ...
            var reportStartDate = getMonthFirstDate(reportEndingDate);
            log.debug('reportStartDate', reportStartDate);
            log.debug('reportEndingDate', reportEndingDate);
            ...
            var totalMtdAvg = 0;
            var totalMtdShipments = 0;
            var totalRunRateShipments = 0;
            var totalVarPercentageVsAct = 0;
            var totalVarAmountVsAct = 0;
            ...
            var salesorderSearchObj = search.create({
                ...
                [
                    ...
                    "AND", 
                    ["fulfillingtransaction.trandate","within",reportStartDate,reportEndingDate]
                ],
                columns:
                [
                    search.createColumn({name: "location", summary: "GROUP", label: "Location"}),
                    search.createColumn({name: "formulacurrency", summary: "SUM", formula: "({fulfillingtransaction.quantity}*{rate})/1000", label: "Formula (Currency)"}),
                    search.createColumn({name: "formulacurrency", summary: "SUM", formula: "({fulfillingtransaction.quantity}*{rate})/1000", label: "Formula (Currency)"})
                    //search.createColumn({name: "formulacurrency", summary: "SUM", formula: "ROUND({amount}/1000,2)", label: "MTD D Avg"}),
            ...
            salesorderSearchObj.run().each(function(result){
                ...
                var salesorderSearchObj = search.create({
                    ...
                    [
                        ...
                        ["applyingtransaction.status","anyof","ItemShip:C"], 
                        "AND", 
                        ["fulfillingtransaction.trandate","on",reportEndingDate],
                        "AND", 
                        ["location","anyof",locationRecordId]
                    ],
                    columns:
                ...
                if(mtdAvg != 0){
                    mtdAvg = mtdAvg / mtdWorkDays;                      
                    runRateShipments = mtdAvg * totalMonthWorkDays;
                }

                var varPercentageVsActLine = '';

                //if(mtdBudget && mtdShipments != 0){
                if(mtdBudget && mtdShipments >= 1){                    
                    varPercentageVsAct = ((mtdBudget - mtdShipments)/mtdBudget)*100;
                    varAmountVsAct = (mtdBudget - mtdShipments);
                ...
                totalShipments = totalShipments + Number(shipments);
                totalMtdAvg = totalMtdAvg + Number(mtdAvg);
                totalMtdShipments = totalMtdShipments + Number(mtdShipments);
                totalRunRateShipments = totalRunRateShipments + Number(runRateShipments);
            ...
            //if(mtdBudget && totalMtdShipments != 0){
            if(mtdBudget && totalMtdShipments >= 1){                    
                totalVarPercentageVsAct = ((mtdBudget - totalMtdShipments)/mtdBudget)*100;
                totalVarAmountVsAct = (mtdBudget - totalMtdShipments);
            ...
            html += "<tr>";
            html += "<td class='bottomborder'><strong>TOTAL</strong></td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalShipments) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalMtdAvg) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalMtdShipments) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalRunRateShipments) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(fullMonthBudgetNew) + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(mtdBudget) + "</td>";
            html += "<td class='bottomborder' align='right'>" + totalVarPercentageVsActLine + "</td>";
            html += "<td class='bottomborder' align='right'>" + formatTotalNumber(totalVarAmountVsAct) + "</td>";
        ...
        function getMonthFirstDate(dateString) {
            ...
            var monthFirstDate = month + '/01/' + year;
        ...
        function formatNumber(value) {
            var formattedNumber = Math.round(Number(value));
